...
#load("./Processed data/sampled mirt models")

# to.be.saved <- list(name = dataset$name, model = model.fit, parameters = parameters, round = round)

sampled.path <- "./Processed data/sampled mirt models"
sampled.mirt.models <- list.files(path = sampled.path, pattern = "*.RData")
...
for(sampled in sampled.mirt.models){
  sampled <- sampled.mirt.models[1]
  paste0("./Processed data/sampled mirt models/", sampled, sep = "") %>% 
   load()
